...
public class EventDistributorTest {
    ...
    @Before
    public void setup() {
        tcpWriter = mock(TcpWriter.class);
        matchingEngine = mock(MatchingEngine.class);
        clientRepository = new ClientRepository();
        eventDistributor = new EventDistributor(tcpWriter, matchingEngine, clientRepository);
    }
    ...
    private void verifyOrderTraded(int clientId, int price, long quantity, Side side) {
        ArgumentCaptor<OrderTraded> captor = ArgumentCaptor.forClass(OrderTraded.class);
        verify(tcpWriter).orderTraded(captor.capture());
        OrderTraded value = captor.getValue();
        assertEquals(clientId, value.getClientId());
        assertEquals(price, value.getPrice());
        assertEquals(quantity, value.getQuantity());
        assertEquals(side, value.getSide());
    }
    ...
    private void verifyOrderRejected(String message) {
        ArgumentCaptor<OrderRejected> captor = ArgumentCaptor.forClass(OrderRejected.class);
        verify(tcpWriter).orderRejected(captor.capture());
        OrderRejected value = captor.getValue();
        assertEquals(message, value.getReason());
    }
